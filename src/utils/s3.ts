...
export const s3Client = bucket && accessKeyId && secretAccessKey
  ? new S3Client({
      region,
...
export async function uploadFile(objectKey: string, body: Buffer, contentType: string): Promise<void> {
...
export async function downloadFile(objectKey: string): Promise<{ body: Buffer; contentType: string }> {
  if (!s3Client || !S3_BUCKET) {
    throw new Error('S3 not configured');
  }
