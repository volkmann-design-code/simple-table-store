...
export const DashboardPage: FC<DashboardPageProps> = ({ session, datastores, lang = 'en' }) => {
  return (
    <Layout title="Dashboard - Datastore" lang={lang}>
      <div class="min-h-screen">
          ...
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
              <div class="flex items-center gap-3">
                ...
                <span class="font-semibold text-surface-100">Datastore</span>
              </div>
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div class="mb-8">
            <h1 class="text-2xl font-bold text-surface-100">{t(lang as 'en' | 'de', 'dashboard.title')}</h1>
            <p class="text-surface-400 mt-1">{t(lang as 'en' | 'de', 'dashboard.subtitle')}</p>
          </div>
          ...
          {datastores.length === 0 ? (
            <div class="card text-center py-12">
              ...
              <h3 class="text-lg font-medium text-surface-200">{t(lang as 'en' | 'de', 'dashboard.noDatastores')}</h3>
              <p class="text-surface-400 mt-1">{t(lang as 'en' | 'de', 'dashboard.contactAdmin')}</p>
            </div>
          ) : (
              ...
              {datastores.map((ds) => (
                ...
                >
                  <div class="flex items-start justify-between">
                    ...
                    <svg class="w-5 h-5 text-surface-500 group-hover:text-primary-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {ds.description && (
                    <p class="mt-1 text-sm text-surface-400 line-clamp-2">{ds.description}</p>
                  )}
                  <div class="mt-3 flex items-center gap-2 text-xs text-surface-500">
                    <span class="px-2 py-0.5 bg-surface-800 rounded">{ds.column_definitions.length} {t(lang as 'en' | 'de', 'dashboard.columns')}</span>
                    <span class="px-2 py-0.5 bg-surface-800 rounded font-mono">{ds.slug}</span>
                  </div>
                </a>
              ))}
            </div>
