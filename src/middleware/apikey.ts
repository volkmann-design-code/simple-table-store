...
function getApiKey(c: { req: { header: (name: string) => string | undefined; query: (name: string) => string | undefined } }): string | undefined {
  return c.req.header('X-API-Key') || c.req.query('api_key');
}

// Helper to validate API key and return datastore ID
async function validateApiKey(apiKey: string): Promise<{ datastoreId: string } | null> {
...
export function verifyApiKeyDatastoreAccess(datastoreId: string, apiKeyDatastoreId?: string): boolean {
