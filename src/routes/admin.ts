...
adminRoutes.post("/users", async (c) => {
	...
	if (!body.email || !body.password || !body.org_id) {
		return c.json(
			{ error: tFromContext(c, "errors.emailPasswordOrgRequired") },
			400,
		);
	}
...
adminRoutes.get("/users/:id", async (c) => {
	const id = c.req.param("id");
	const user = await adminQueryOne<User>("SELECT * FROM users WHERE id = $1", [
		id,
	]);
	...
	try {
       ...
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
			[
				...
				JSON.stringify(body.column_definitions),
			],
		);
...
adminRoutes.get("/api-keys", async (c) => {
	...
	if (!datastoreId) {
		return c.json(
			{ error: tFromContext(c, "errors.datastoreIdRequired") },
			400,
		);
	}
...
adminRoutes.post("/api-keys", async (c) => {
	...
	if (!body.datastore_id || !body.name) {
		return c.json(
			{ error: tFromContext(c, "errors.datastoreIdNameRequired") },
			400,
		);
	}
...
adminRoutes.get("/migrate/status", async (c) => {
	try {
		...
		const message =
			error instanceof Error ? error.message : "Failed to get migration status";
		return c.json(
			{
				error: message,
			},
