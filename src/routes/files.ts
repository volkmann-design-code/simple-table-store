...
import { withUserContext, withApiKeyContext } from '../db';
import { uploadFile, deleteFile, S3_BUCKET } from '../utils/s3';
import { getCachedFile } from '../utils/file-cache';
import { tFromContext } from '../i18n';
import type { File, DataStore, FileReference } from '../types';
...
async function getApiKeyDatastoreId(c: { req: { header: (name: string) => string | undefined; query: (name: string) => string | undefined } }): Promise<string | null> {
  ...
  const key = await adminQueryOne<{ datastore_id: string; expires_at: Date | null }>(
...
fileRoutes.post('/datastores/:slug/files', sessionAuth, async (c) => {
  ...
  if (!S3_BUCKET) {
    return c.json({ error: tFromContext(c, 'errors.fileUploadsNotConfigured') }, 503);
  }
  ...
  const result = await withUserContext<{ datastore: DataStore | null; fileRecord: File | null }>(
    session.userId,
    session.orgId,
    async (client) => {
      // Get datastore
  ...
  if (!result.fileRecord) {
    return c.json({ error: tFromContext(c, 'errors.failedToCreateFileRecord') }, 500);
  }
...
fileRoutes.get('/files/:id', async (c) => {
  ...
  if (!file) {
    return c.json({ error: tFromContext(c, 'errors.fileNotFoundOrAccessDenied') }, 404);
  }

  // Get file from cache or S3
  const cached = await getCachedFile(file.object_key);
  if (!cached) {
    return c.json({ error: tFromContext(c, 'errors.fileNotFoundInStorage') }, 404);
  }
...
fileRoutes.delete('/files/:id', sessionAuth, async (c) => {
  ...
  const result = await withUserContext<{ file: File | null; deleted: boolean }>(
    ...
    async (client) => {
      // Get file
      const fileResult = await client.query('SELECT * FROM files WHERE id = $1', [fileId]);
