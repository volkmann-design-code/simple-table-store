...
import { verifyApiKeyDatastoreAccess } from '../middleware/apikey';
import { withUserContext, withApiKeyContext, adminQueryOne } from '../db';
...
import { tFromContext, getLanguage } from '../i18n';
import type { DataStore, DataRecord, ApiKey, PaginatedResponse } from '../types';

export const apiRoutes = new Hono();

// Helper to check API key and get datastore ID
async function getApiKeyDatastoreId(c: { req: { header: (name: string) => string | undefined; query: (name: string) => string | undefined } }): Promise<string | null> {
...
apiRoutes.get('/datastores/:slug/records', async (c) => {
  ...
  if (apiKeyDatastoreId) {
    // API key auth - use API key context
        const result = await withApiKeyContext<{ records: DataRecord[]; total: number; datastore: DataStore | null }>(
      apiKeyDatastoreId,
      async (client) => {
  ...
  if (!token) {
    return c.json({ error: tFromContext(c, 'errors.authenticationRequired') }, 401);
  }
  ...
  const result = await withUserContext<{ records: DataRecord[]; total: number; datastore: DataStore | null }>(
    session.userId,
    session.orgId,
    async (client) => {
...
apiRoutes.post('/datastores/:slug/records', sessionAuth, async (c) => {
  ...
  const result = await withUserContext<{ record: DataRecord | null; errors?: { field: string; message: string }[] }>(
    session.userId,
    session.orgId,
    async (client) => {
      ...
      const errors = validateRecordData(body.data, datastore.column_definitions, lang);
      if (errors.length > 0) {
  ...
  if (result.errors) {
    return c.json({ error: tFromContext(c, 'errors.validationFailed'), details: result.errors }, 400);
  }
...
apiRoutes.patch('/datastores/:slug/records/:id', sessionAuth, async (c) => {
  ...
  const result = await withUserContext<{ record: DataRecord | null; errors?: { field: string; message: string }[]; notFound?: boolean }>(
    session.userId,
    session.orgId,
    async (client) => {
      // Get datastore
...
apiRoutes.delete('/datastores/:slug/records/:id', sessionAuth, async (c) => {
  ...
  const deleted = await withUserContext<boolean>(
    ...
    async (client) => {
      ...
      const record = recordResult.rows[0] as DataRecord;
      const datastoreResult = await client.query('SELECT * FROM datastores WHERE id = $1', [datastoreId]);
